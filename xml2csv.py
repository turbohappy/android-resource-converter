...
search_dir = directory_with_android_resources + "/**/strings.xml"
xml_files = glob.glob(search_dir, recursive=True)
...
for xml_file in xml_files:
    ...
    xml_parsed_files.append(minidom.parse(xml_file).getElementsByTagName('string'))
...
for translation_key in sorted(translations_dictionary, key=lambda k: len(translations_dictionary[k]), reverse=True):
    ...
    for translation in translations_dictionary[translation_key]:
        entersRemoved = " ".join(translation.split('\n'))
